#!/bin/bash
#SBATCH --job-name=random_small36
...
#SBATCH --output=logs/random_small36_%j.out
#SBATCH --error=logs/random_small36_%j.err
...
echo "🎲 Creating and training probes on RANDOM WEIGHTS small-36 model"
echo "Starting at $(date)"

# Create properly randomized version of small-36 model
python -c "
import torch
import numpy as np
...
# Load original model
original_path = 'models/tf_lens_small-36-600k_iters.pth'
random_path = 'models/tf_lens_small-36_RANDOM.pth'

print('🎲 Loading original small-36 model...')
...
echo "🧪 Training probes on randomized small-36 model..."
...
python train_test_chess.py \
    ...
    --model_name tf_lens_small-36_RANDOM \
    --n_layers 36 \
    --first_layer 0 \
    --last_layer 35

echo "✅ Random small-36 probe training completed at $(date)"